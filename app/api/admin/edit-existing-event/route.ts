...
interface EditExistingEventRequest {
  eventId: string;
  title: string;
  startDate: string;
  startTime?: string;
...
export async function PUT(request: Request) {
  try {
    ...
    const eventData: EditExistingEventRequest = await request.json()
    ...
    console.log('✏️ Admin editing existing event:', eventData.eventId)
    ...
    console.log('📅 Updating existing event in Google Calendar:', eventData.eventId);
    ...
    console.log('✅ Existing event updated in Google Calendar');
    ...
    console.error('❌ Error editing existing event:', error);
    ...
    return NextResponse.json(
      { error: 'Failed to update existing event', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}