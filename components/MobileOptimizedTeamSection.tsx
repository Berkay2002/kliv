...
interface MobileOptimizedTeamSectionProps {
  members: Member[];
}
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {isMobile ? (
          <div className="relative">
                  ...
                  <div key={member.id} className="w-full flex-shrink-0 px-4">
                    ...
                    >
                      ...
                      <div className="p-6 text-center">
                        ...
                        <span className={`inline-block px-3 py-1 mb-2 rounded-full text-sm font-semibold ${
                        ...
                        {member.description && (
                          <p className="text-muted-foreground mb-2 text-sm">{member.description}</p>
                        )}
                        {member.email && (
                          <Link
            ...
            <button
              onClick={prevSlide}
              className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-card hover:bg-kliv-red/10 border border-border hover:border-kliv-red/30 rounded-full flex items-center justify-center transition-colors duration-200 shadow-sm"
              aria-label="Previous team member"
            >
              <ChevronLeft className="w-5 h-5 text-kliv-red" />
            </button>
            <button
              onClick={nextSlide}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-card hover:bg-kliv-red/10 border border-border hover:border-kliv-red/30 rounded-full flex items-center justify-center transition-colors duration-200 shadow-sm"
              aria-label="Next team member"
            >
              <ChevronRight className="w-5 h-5 text-kliv-red" />
            </button>
              ...
              {members.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all duration-200 ${
                    index === currentSlide
                      ? 'bg-kliv-red scale-125'
                      : 'bg-muted-foreground/30 hover:bg-kliv-red/50'
                  }`}
            ...
            <div className="text-center mt-4">
              <span className="text-sm text-muted-foreground">
                {currentSlide + 1} av {members.length}
              </span>
            </div>
          </div>
        ) : (
